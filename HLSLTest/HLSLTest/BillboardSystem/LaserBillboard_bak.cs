...
namespace HLSLTest
{
	public class LaserBillboard : Drawable
	{
			...
			Vector3 Right, Vector3 CameraPosition)
		{
			...
			//Vector3 projectedStart = view.Project(Start, Projection, View, Matrix.CreateTranslation(Start));
			//Vector3 projectedEnd = view.Project(End, Projection, View, Matrix.CreateTranslation(End));


			//Matrix.CreateWorld(End, End - Start, Vector3.Cross(Up, Right)));
			...
			Vector3 size = projectedEnd - projectedStart;
			size /= 2.0f;
			//effect.Parameters["Size"].SetValue(new Vector2((float)Math.Abs(size.X), (float)Math.Abs(size.Y)));
			effect.Parameters["Size"].SetValue(new Vector2((float)Math.Abs(size.X), billboardSize.Y));
